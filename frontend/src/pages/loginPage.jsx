    ...
    return (
        ...
        <LoginForm>
            <form className="login-form" ref={formRef} onSubmit={onSubmit}>
                ...
                <input type="text" className="login-input email" onChange={emailInput} placeholder="Your email adress"/>
                ...
                <input type="password" className="login-input password" onChange={passwordInput} placeholder="Your password"/><br/>
                <p className="text">{password.length > 0 && <span className={`message ${isPassword  ? 'success' : 'error'}`}>{passwordMessage}</span>}</p>
                <button className="login-button" disabled={disabled}>로그인</button>
